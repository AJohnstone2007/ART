...
public class Rewriter {
  ...
  private int rewriteAttempt(int term, int relationTerm, int level) { // return rewritten term and set rewriteAttemptOutcome
    ...
    if (tr.isTerminatingConfiguration(term, relationTerm)) {
      Util.trace(4, level + 1, "Terminal " + ScriptInterpreter.iTerms.toString(term));
      return term;
    }
    ...
    if (rulesForThisRelation != null) { // There may be no rules at all in this relation!
      ruleList = rulesForThisRelation.get(ScriptInterpreter.iTerms.termSymbolStringIndex(rootTheta));
      if (ruleList == null) ruleList = rulesForThisRelation.get(ScriptInterpreter.iTerms.findString("")); // Default rules - 1 Oct 24 What are these?
    }
    ...
    nextRule: for (int ruleIndex : ruleList) {
      ...
      int rhs = ScriptInterpreter.iTerms.subterm(ruleIndex, 1, 1, 2);
      ...
      for (int premiseNumber = 0; premiseNumber < premiseCount; premiseNumber++) {
        ...
        if (ScriptInterpreter.iTerms.hasSymbol(premise, "trMatch")) {// |> match expressions
          ...
          if (ScriptInterpreter.iTerms.hasSymbol(premise, "trTransition")) {
            int rewriteTerm = ScriptInterpreter.iTerms.substitute(bindings, tr.thetaLHSFromConfiguration(premise), 0);
