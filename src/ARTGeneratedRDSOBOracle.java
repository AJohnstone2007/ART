...
class ARTGeneratedRDSOBOracle extends uk.ac.rhul.cs.csle.art.cfg.rdsob.RDSOBAbstract {
  ...
  boolean parse_S() {
    ...
    inputIndex = iiAtEntry;
    oracleIndex = oiAtEntry;
    oracleSet(1);
    if (parse_e1()) {
      return true;
    }
  ...
  void parse(String filename) throws FileNotFoundException {
    ...
    builtIn_WHITESPACE();
    if (!(parse_S() && input.charAt(inputIndex) == '\0')) {
    ...
    inputIndex = oracleIndex = 0;
    builtIn_WHITESPACE();
    term_S(new LinkedList<>());
  }

  public static void main(String[] args) throws FileNotFoundException {
    if (args.length < 1)
      System.out.println("Usage: java ARTGeneratedRDSOBOracle <input file name>");
    else
      new ARTGeneratedRDSOBOracle().parse(args[0]);
