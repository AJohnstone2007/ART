...
{
  unsigned long sum = hist_sum_buckets(this_hist);
  ...
  while (this_histogram_node->next != NULL)
  {
    if (this_histogram_node->value != 0)
      text_printf("%lu: %lu %.2f\n", this_histogram_node->bucket, this_histogram_node->value, ((double) this_histogram_node->value / (double) sum) * 100.0);
