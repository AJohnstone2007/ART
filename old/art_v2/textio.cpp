...
const char* text_bot = NULL;       /* text array for storing id's and strings */
...
static const char* name = NULL;    /* filename */
...
{
  const char* name;                /* copy of filename */
...
const char* text_capitalise_string(char* str)
{
  char* ret = str;
...
static const char* text_ASCII_table[] = {
...
const char* text_make_C_identifier(const char* str)
{
  ...
  while (* temp != 0)
    length += strlen(text_ASCII_table[(unsigned) *temp++]);
  ...
  * temp = 0;                 /* make temp an empty string */
  ...
  while (* str != 0)
    strcat(temp, text_ASCII_table[(unsigned) *str++]);
...
void text_init(const long max_text, const unsigned max_errors, const unsigned max_warnings, const unsigned tab_width)
{
  ...
  text_bot =(const char* ) mem_malloc(maxtext);  /* allocate text buffer */
  
  text_top = (char*) text_bot;        /* top of text character */
  text_current = last_char = first_char = (char*) text_bot + maxtext;  /* make new buffer region below top of text */
}

const char* text_insert_char(const char c)
{
  const char* start = text_top; 
...
const char* text_insert_substring(const char* prefix, const char* str, const unsigned n) /* put an id_number into text buffer */
{
  const char* start = text_top; 
...
const char* text_lowercase_string(char* str)
{
  char* ret = str;
...
int text_print_as_C_string_or_char(FILE * file, const char* string, int is_char_string)
{
  ...
  if (*string == '\0') /* special case of zero length string */
  {
    putc('\\', file); putc('0', file);
    return 2;
  }
...
const char* text_uppercase_string(char* str)
{
  const char* ret = str;
...
const char* text_strdup(const char* str)  /* Make a copy of a string, like Unix strdup() */
{
