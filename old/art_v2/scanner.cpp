...
{                               /* get the next lexeme */
  ...
  int last = ' ';
    ...
    {
      const char *temp = strdup(set_return_element(SCAN_CAST->token, scan_token_names));
...
//      char *tmp = ((scan_data*) text_scan_data)->id;
      while (text_char != *(SCAN_CAST->id))
      {
        if ( /* text_char == '\n' || */ text_char == EOF)
        {
