...
const char *lexBuiltinWhitespace(const char *start);
  ...
  bool lexLongestMatch(const char *parseString) {
    ...
    while (1) {
      ...
      for (artLabel temp = (artLabel) 1; temp != ART_EPSILON; temp = (artLabel) ((int) temp + 1)) {
...
#if defined(TRACE)
  cout << "Token string:\n";
  for (artLabel *tmp = tokenString; *tmp != ART_EOS; tmp++) {
...
#if defined(LOCAL_BUILTINS)
#include "localbuiltins.h"
#endif
