...
extern const char* rdp_sourcefilename;
...
void scan_insert_comment_block(const char *pattern, unsigned long column, unsigned long sequence_number);
unsigned long scan_line_number(void);
void scan_lexicalise(void);
void scan_load_keyword(const char * id1, const char * id2, const int token, const int extended);
...
int scan_token_number(const char *keyword);
