...
script_symbols_data *script_symbol_new(char *id)
{
  return (script_symbols_data *) symbol_find(script_symbols, &id, sizeof(char *), sizeof(script_symbols_data), NULL, SYMBOL_NEW);
}
...
static script_value script_built_in_augment_grammar(void *root, int side_effects)
{
  ...
  if (side_effects)
  {
    ...
    return_value.type = SCRIPT_P_VOID;
    gram_tidy(this_value.v_GRAMMAR, 2, 1);     /* force augmentation */
  }
...
static script_value script_built_in_earley_parse(void *root, int side_effects)
{
  ...
  if (side_effects)
  {
    ...
    script_get_any_parameter(&this_edge, &this_string, side_effects);

    if (this_string.type == SCRIPT_P_STRING)
      ;
    else if (this_string.type == SCRIPT_P_FILE)
    {
      this_string.type == SCRIPT_P_STRING;

      this_string.v_STRING = script_string_from_file(this_string.v_FILE);
    }
    else
    {
        text_message(TEXT_FATAL, "type mismatch in call to function 'earley_parse': expected type STRING or FILE but found '%s'\n",
                     script_primitive_type_string(this_string.type));
        this_string.v_STRING = "";
    }

    if (this_edge != NULL)
    {
      script_get_parameter(&this_edge, &this_value, SCRIPT_P_BUILT_IN_ENUM, "earley_parse", side_effects);

      switch (this_value.v_INTEGER)
      {

        case SCRIPT_BUILT_IN_earley_lookahead:
          lookahead = 1;
          break;

        case SCRIPT_BUILT_IN_earley_no_lookahead:
          lookahead = 0;
          break;

        default: text_message(TEXT_FATAL, "earley_parse parameter 3: expecting one of 'earley_lookahead' or 'earley_no_lookahead'\n");
      }
    }

    return_value.type = SCRIPT_P_DERIVATION;
    return_value.v_DERIVATION = gp_earley_parse(this_grammar.v_GRAMMAR, this_string.v_STRING, lookahead);
  }

  return return_value;
}

static script_value script_built_in_earley_recognise(void *root, int side_effects)
{
  script_value return_value;
  void *this_edge = graph_next_out_edge(root);
  script_value this_value;
  ...
  if (side_effects)
  {
    script_get_parameter(&this_edge, &this_grammar, SCRIPT_P_GRAMMAR, "earley_recognise", side_effects);
    ...
    {
        text_message(TEXT_FATAL, "type mismatch in call to function 'earley_recognise': expected type STRING or FILE but found '%s'\n",
                     script_primitive_type_string(this_string.type));
        this_string.v_STRING = "";
    }
    ...
    if (this_edge != NULL)
    {
      script_get_parameter(&this_edge, &this_value, SCRIPT_P_BUILT_IN_ENUM, "earley_recognise", side_effects);
      ...
      switch (this_value.v_INTEGER)
      {
        ...
        default: text_message(TEXT_FATAL, "earley_recognise parameter 3: expecting one of 'earley_lookahead' or 'earley_no_lookahead'\n");
      }
    }

    return_value.type = SCRIPT_P_DERIVATION;
    return_value.v_DERIVATION = gp_earley_recognise(this_grammar.v_GRAMMAR, this_string.v_STRING, lookahead);
  }
...
static script_value script_built_in_gll_parse(void *root, int side_effects)
{
  script_value return_value;
  void *this_edge = graph_next_out_edge(root);
  script_value this_value;
  ...
  if (side_effects)
  {
    script_get_parameter(&this_edge, &this_grammar, SCRIPT_P_GRAMMAR, "gll_parse", side_effects);
    script_get_parameter(&this_edge, &this_value, SCRIPT_P_STRING, "gll_parse", side_effects);
    script_get_parameter(&this_edge, &this_string, SCRIPT_P_STRING, "gll_parse", side_effects);

    return_value.type = SCRIPT_P_VOID;
    pred_gll_parse(this_grammar.v_GRAMMAR, this_value.v_STRING, this_string.v_STRING);
  }
...
static script_value script_built_in_brnglr_parse(void *root, int side_effects)
{
  script_value return_value;
  dfa *this_dfa;
  void *this_edge = graph_next_out_edge(root);
  ...
  if (side_effects)
  {
    script_get_parameter(&this_edge, &this_value, SCRIPT_P_DFA, "brnglr_parse", side_effects);
    this_dfa = this_value.v_DFA;
    ...
    {
        text_message(TEXT_FATAL, "type mismatch in call to function 'brnglr_parse': expected type STRING or FILE but found '%s'\n",
                     script_primitive_type_string(this_value.type));
        this_value.v_STRING = "";
    }
    ...
    return_value.type = SCRIPT_P_DERIVATION;
    return_value.v_DERIVATION = sr_brnglr_parse(this_dfa, this_value.v_STRING, 0);
  }
...
void script_initialise(char *sourcefilename)
{
  ...
  script_make_built_in_function("earley_recognise", script_built_in_earley_recognise);
  ...
  script_make_built_in_function("gll_parse", script_built_in_gll_parse);
  ...
  script_make_built_in_function("brnglr_parse", script_built_in_brnglr_parse);
  ...
  script_make_built_in_enum("earley_lookahead", SCRIPT_BUILT_IN_earley_lookahead);
  script_make_built_in_enum("earley_no_lookahead", SCRIPT_BUILT_IN_earley_no_lookahead);
