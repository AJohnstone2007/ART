/*******************************************************************************
*
* Parser generated by RDP on Jul 10 2024 19:47:16 from gtb.bnf
*
*******************************************************************************/
#include <time.h>
#include "gtb_scr.h"
...
/* Set declarations */
  ...
  set_ lhs_rule_name_stop = SET_NULL;
...
RDP_T_9142 /* [* */, RDP_T_96 /* ` */, RDP_T_124 /* | */, RDP_T_126 /* ~ */,SET_END);
  set_assign_list(&lhs_rule_name_stop, SCAN_P_EOF, RDP_T_585861 /* ::= */,SET_END);
...
RDP_T_96 /* ` */, RDP_T_126 /* ~ */, SET_END);
  set_assign_list(&rdp_rule_name_2_first, SCAN_P_ID, RDP_T_126 /* ~ */, SET_END);
  ...
  set_assign_list(&rdp_text_2_first, SCAN_P_ID, RDP_T_40 /* ( */, SET_END);
  set_assign_list(&rule_stop, SCAN_P_ID, SCAN_P_EOF, RDP_T_40 /* ( */,SET_END);
...
RDP_T_12461 /* |= */, RDP_T_126 /* ~ */,SET_END);
  set_assign_list(&text_first, SCAN_P_ID, RDP_T_40 /* ( */, SET_END);
...
static char* lhs_rule_name(rdp_tree_node_data* rdp_tree);
...
static char* lhs_rule_name(rdp_tree_node_data* rdp_tree)
{
  ...
  {
    if (rdp_tree_update) memcpy(rdp_tree, text_scan_data, sizeof(scan_data));
    scan_test(NULL, SCAN_P_ID, &lhs_rule_name_stop);
    result = SCAN_CAST->id;
    scan_();
    if (rdp_pass == 2) { \
 rdp_tree->symbol_table_entry = script_symbol_find(result);\
    ...
    scan_test_set(NULL, &lhs_rule_name_stop, &lhs_rule_name_stop);
   }
  return result;
}
...
static void rule(rdp_tree_node_data* rdp_tree)
{
 ...
 script_open_scope();  \
    ...
    if(rdp_tree_update) {rdp_tree->id = "lhs_rule_name"; rdp_tree->token = 0;}
name = lhs_rule_name(rdp_tree);
    if (rdp_pass == 2) { \
 script_name_scope(name);  \
}if (rdp_pass == 1) { \
 script_symbol_new(name);  \
}
...
int main(int argc, char *argv[])
{
  ...
  arg_message("GTB V3.0 (c) Adrian Johnstone 1997-2024\n" RDP_STAMP "\n\n""Usage: gtb [options] source[.gtb]");
  ...
  arg_string ('C', "Add comment prefix to log line", &c_value);
