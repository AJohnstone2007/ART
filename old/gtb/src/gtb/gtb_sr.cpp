...
void sr_ssg_statistics(FILE * output_file, void *ssg_trace)
{
  ...
  text_printf("\nSSG has final level %u with %u node%s and %u edge%s\n",
              max_level,
              node_count, node_count == 1 ? "" : "s",
              edge_count, edge_count == 1 ? "" : "s"
              );
...
void vcg_print_ssg_node(const void *node)
{
  ...
  if (this_node->value >= sr_current_render_derivation->dfa->state_count + sr_current_render_derivation->dfa->nfa->grammar->first_dfa_state)
  {
    level--;
    text_printf("borderwidth:2 color:magenta label:\"%i:%u\"",  this_node->value, graph_atom_number(this_node));
  }
  else if (GRAM_IS_DFA_STATE(sr_current_render_derivation->dfa->nfa->grammar, (this_node->value)))
  {
    level--;
