/***************************************************************************
*
* gramex version 3.4 by Adrian Johnstone (A.Johnstone@rhul.ac.uk)
*
...
* V3.1 modified output format to use ()? and ()* when [] and {} are EBNF
* V3.2 blank lines after LHS nonterminal are set as continuation lines for the JLS document
* V3.3 tightened up scanner so that metacharacters are always separate tokens
* V3.4 added -t option, repaired one-of and implemented xOpt where x is a token
*
...
enum lineKind {K_COMMENT, K_MAYBE_TYPO, K_PASCAL_START_RULE, K_PASCAL_START_END_RULE,
...
enum scannerKind {S_NL, S_ID, S_META, S_OTHER};
...
int twelve_alternates_max = 0;
char *scan_start, *scan_end;
enum scannerKind token;
int first_production_indent;

char printable(char c) {
  return isprint(c) ? c : (char) ' ';
}
...
void scan(void)
{
  ...
  if (*scan_end == '\n') {
    ...
    printf("*scan_start = '%c'; *scan_end = '%c'; length = %i; Scanned new line\n", printable(*scan_start), printable(*scan_end), scan_end - scan_start);
#endif
    token = S_NL;
    return;
  }

  // In EBNF mode, return leading meta characters as single tokens
  if (isMeta(*scan_end)) {
    scan_end = scan_start + 1;
#if defined (SCAN_TRACE)
    printf("*scan_start = '%c'; *scan_end = '%c'; length = %i; Scanned meta character %.*s\n", printable(*scan_start), printable(*scan_end), scan_end - scan_start, scan_end - scan_start, scan_start);
#endif
    token = S_META;
    return;
  }
  ...
  if (isalpha(*scan_end)) {
    while (isalpha(*scan_end) || isdigit(*scan_end))
      scan_end++;

#if defined (SCAN_TRACE)
    printf("*scan_start = '%c'; *scan_end = '%c'; length = %i; Scanned identifier %.*s\n", printable(*scan_start), printable(*scan_end), scan_end - scan_start, scan_end - scan_start, scan_start);
#endif
    token = S_ID;
    return;
  }

  while (!isspace(*scan_end) && !isalpha(*scan_end) && !isMeta(*scan_end))
    scan_end++;

  // special case: check for opt
  if (*(scan_end) == 'o' &&
      *(scan_end + 1) == 'p' &&
      *(scan_end + 2) == 't' &&
      !isalpha(*(scan_end + 3))
...
#if defined (SCAN_TRACE)
  printf("*scan_start = '%c'; *scan_end = '%c'; length = %i; Scanned other %.*s\n", printable(*scan_start), printable(*scan_end), scan_end - scan_start, scan_end - scan_start, scan_start);
#endif
  token = S_OTHER;
}
...
{
  if (use_ebnf)
    return 0;  // Don't mix conventions!
...
void help(void)
{
  printf("gramex V3.1 (c) Adrian Johnstone 2006, 2007, 2012\n\n"
...
//    printf("\n*** %i ***\n", line_number);
    ...
    /* mapping applied at start of line only */
    test_and_replace(current, "[ ", "( ");
    test_and_replace(current, "f ", "( ");  /* { comes out as f */
    test_and_replace(current, "g ", ")*");  /* } comes out as g */
    test_and_replace(current, "j ", "| ");  /* | comes out as | */
    test_and_replace(current, "? ", "> ");  /* ? (conformant levels( comes out as ? */
    test_and_replace(current, "] ", ")?");
    test_and_replace(current, ". ", "; ");
...
void process_c(void)
{
  ...
  /* Look for rules starts: an ID followed by a colon on a line of its own */
  for (temp = 0; temp < linecount - 1; temp++)
  {
    ...
    lines[temp].lhs_end = scan_end;

    scan();

    if (!(scan_start == scan_end - 1 && *scan_start == ':'))
      continue;
  ...
  for (temp = 0; temp < linecount - 1; temp++)
  {
    ...
    if (lines[temp].kind == K_BLANK)
      if (!(lines[temp-1].kind == K_START_RULE || lines[temp-1].kind == K_START_RULE_ONE_OF))   // The JLS often has a blank line after the initial ID: RHS designator
        in_rule = 0;

    if (twelve_alternates_max && alternate_count == 12)
      in_rule = 0;
  ...
  for (temp = 1; temp < argc; temp++)
  {
    if (*argv[temp] == '-')  /* option */
      switch (*(argv[temp] + 1))
      {
        ...
        case 't': twelve_alternates_max = 1; break;
        default:
...
//      fprintf(stderr, "suppressing section at line %i\n", temp);
  ...
  /* Output lines */
  printf("// Generated by gramex V3.4 from '%s' (this build "
