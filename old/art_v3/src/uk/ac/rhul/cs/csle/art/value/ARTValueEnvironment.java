...
public class ARTValueEnvironment extends ARTValueCollection {
  ...
  public ARTValue insertKey(ARTValue key, ARTValue value) throws ARTValueException {
    if (payload.containsKey(key)) throw new ARTValueException("ARTValueEnvironment insertion of key that is already in the local map" + key);
  ...
  public ARTValue deleteKey(ARTValue key) throws ARTValueException {
    if (!payload.containsKey(key)) throw new ARTValueException("ARTValueEnvironment deletion of key that is not in the local map" + key);
    payload.remove(key);
    return this;
  }
  ...
  public ARTValue update(ARTValue key, ARTValue value) throws ARTValueException {
    if (!payload.containsKey(key))
      throw new ARTValueException("ARTValueEnvironment update of key that is not in the local map: " + key + "\nAvailable parameters are: " + payload.keySet());
    payload.put(key, value);
    return this;
  }
  ...
  public ARTValue updateOrdered(ARTValueCollection value) throws ARTValueException {
    ...
    if (loadIterator.hasNext()) throw new ARTValueException("update ordered has more update elements than collection elements");
  ...
  public ARTValue valueKey(ARTValue key) throws ARTValueException {
    ...
    throw new ARTValueException("ARTValueEnvironment lookup of unknown key: " + key);
  }
}
