package uk.ac.rhul.cs.csle.art.alg.earley.linkedapi;
...
public class ARTEarleyLinkedAPI extends ARTParserBase {
  ...
  private boolean inSigmaN(ARTGrammarInstance instance) throws ARTException {
    if (instance instanceof ARTGrammarInstanceCat) instance = instance.getChild();

    if (!(instance instanceof ARTGrammarInstanceSlot)) throw new ARTException("call to inSigmaN with instance that is neither a cat nor a slot");
  ...
  @Override
  public void artParse(String stringInput, String inputFilename) throws ARTException {
    if (ARTTRACE) artTraceText = new ARTText(new ARTTextHandlerFile(this.getClass().getSimpleName() + "Trace.txt"));
    ...
    ArrayList<ARTGrammarElementTerminal> input = new ARTLexerBase(artGrammar).lexicaliseToArrayListOfTerminals(stringInput, 1);
