package uk.ac.rhul.cs.csle.art.alg.gll;
...
public class ARTGLLGenerator {
  ...
  public ARTGLLGenerator(ARTGrammar grammar, ARTOptionBlock optionBlock) throws ARTException {
    this.grammar = grammar;
    this.optionBlock = optionBlock;
  }

  public void generateParser(ARTText text) throws ARTException {
    ...
    if (optionBlock.mGLL && grammar.isEBNF()) { // Write out inadmissable parser
      gt = new ARTGeneratorTranslateJava(text);
      gt.fileOpen(optionBlock.parserName + ".java", optionBlock.namespace);
      gt.classOpen(optionBlock.parserName, "ARTGLLParserHashPool");
      gt.constructorOpen(optionBlock.parserName, "ARTLexerBase", "artLexerBase");
      gt.functionCall("this", "null", "artLexerBase");
      gt.constructorClose(optionBlock.parserName);
      gt.newLine();

      gt.constructorOpen(optionBlock.parserName, "ARTGrammar", "artGrammar", "ARTLexerBase", "artLexerBase");
      gt.functionCall("super", "artGrammar", "artLexerBase");
      gt.constructorClose(optionBlock.parserName);
      gt.newLine();

      gt.functionVoidOpenThrows("artParseBody", gt.stringName(), "artInputString", gt.integerName(), "artStartLabel");

      gt.assign("artInadmissable", "true");
      gt.functionClose("artParseBody");
      gt.classClose(optionBlock.parserName);
      gt.fileClose(optionBlock.parserName + ".java", optionBlock.namespace);
  ...
  private void generateParserPass(ARTText text) throws ARTException {
    gt = new ARTGeneratorTranslateJava(text);
    gt.indentSet(0);
    gt.fileOpen(optionBlock.parserName + ".java", optionBlock.namespace);
    ...
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool) {
      gt.getText().println();
      gt.indent();
    }
    ...
    writeConstructor(optionBlock.parserName);
    if (gt.targetLanguageName().equals("Java") && optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) { // evaluation presently only available for
                                                                                                                    // Java
      // GLL parsers
      gt.indent();
      gt.getText().printf("private ARTGLLRDT artRDT;\n");
      gt.indent();
      gt.getText().printf("private int artNextFreeNodeNumber = 1;\n");
      if (!optionBlock.suppressSemantics) for (String s : grammar.getSupportStrings())
        gt.getText().printf("%s\n", s);
      writeRDEvaluator();
    }
    gt.indentDown();
  ...
  public void generateLexer(ARTText text) {
    gt = new ARTGeneratorTranslateJava(text);
    gt.indentSet(0);
    gt.fileOpen(optionBlock.lexerName + ".java", optionBlock.namespace);
    gt.classOpen(optionBlock.lexerName, "ARTLexerBase");

    writeLexicaliseBuiltinInstances();
    writeLexicalisePreparseWhitespaceInstances();
    gt.classClose(optionBlock.lexerName);
    gt.fileClose(optionBlock.lexerName + ".java", optionBlock.namespace);
  }
  ...
  void parserBlockOpenSelect(String label, String comment) throws ARTException {
    if (comment != null) {
      gt.indent();
      gt.comment(comment);
    }
    switch (optionBlock.despatchMode) {
    case dynamic:
    case static_:
      gt.label(label);
      break;
    ...
    default:
      throw new ARTException("unknown despatch mode\n");
    }
  }

  void jumpSelect(String id) throws ARTException {
    switch (optionBlock.despatchMode) {
    case dynamic:
      gt.jumpDynamic(id);
      break;
  ...
  String emptyNodeSelect() {
    if (optionBlock.supportMode == ARTModeSupport.ObjectOriented)
      return gt.nullName();
    else
      return "0";
  }
  ...
  void printStaticSwitchElementRec(ARTGrammarInstance node) throws ARTException {
  ...
  void printStaticSwitch() throws ARTException {
  ...
  void writeCodeGLLRec(ARTGrammarInstance iftNode, ARTGrammarInstance lastLHSNode) throws ARTException {
     ...
     */
      ...
      if (optionBlock.despatchMode == ARTModeDespatch.fragment) {
      ...
      for (ARTGrammarInstance childNode = iftNode.getChild(); childNode != null; childNode = childNode.getSibling()) {
        ...
        if (!optionBlock.suppressProductionGuard) {
        ...
        if (!optionBlock.suppressProductionGuard) gt.indentDown();
      ...
      for (ARTGrammarInstance childNode = iftNode.getChild(); childNode != null; childNode = childNode.getSibling()) {
        ...
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool) {
          if (grammar.getNonterminalsDeclaredAsTerminals().contains(iftNode.getPayload())) { // Only do this for lexical nonterminals
            gt.indent();
            gt.getText().println(
        ...
        if (!(optionBlock.predictivePops && iftNode.isPostPredictivePop)) {
          if (!optionBlock.suppressPopGuard) {
     ...
     */
      ...
      if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool)
        gt.functionAssignCall("artCurrentSPPFRightChildNode", "artFindSPPFEpsilon", "artCurrentInputPairIndex");
     ...
     */
    else if (iftNode instanceof ARTGrammarInstanceTerminal)

    {
      ...
      if (optionBlock.mGLL) { // MGLL variannt
        gt.forSuccessorPair();
        gt.newLine();
        ...
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) iftNode.isSPPFLabel = true;
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.functionAssignCall("artCurrentSPPFRightChildNode", "artFindSPPFTerminal",
            iftNode.getPayload().toEnumerationString(null), "artCurrentInputPairIndex", gt.inputSuccessorReferenceLeftExtent());
        iftNode.erL.isSPPFLabel = true;
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool)
        ...
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.functionAssignCall("artCurrentSPPFRightChildNode", "artFindSPPFTerminal",
            iftNode.getPayload().toEnumerationString(null), "artCurrentInputPairIndex", gt.inputAccessLeftExtent());
      ...
      // Add to TWESet
      if (optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool && !(iftNode.getPayload() instanceof ARTGrammarElementTerminalCharacter))
        gt.getText().println("tweSet.tweSetUpdateExactMakeLeftSet(" + iftNode.getPayload().toEnumerationString(null) + ", artCurrentInputPairIndex - "
            + (iftNode.getPayload().toString().length() - 2) /* remove delimiters! */ + ", artCurrentInputPairIndex);");
     ...
     */
      ...
      if (childNode.first.contains(grammar.getEpsilon())) {
        ...
        if (TEMPLATE2016) {
          ...
          if (!optionBlock.suppressTestRepeat) {
            iftNode.isTestRepeatLabel = true;
     ...
     */
      ...
      if (!childNode.first.contains(grammar.getEpsilon())) {
        ...
        iftNode.erL.isCodeLabel = true;
        findGSSSelect(iftNode);
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.assign("artCurrentSPPFNode", "artDummySPPFNode");
...

    {
      ...
      if (!childNode.first.contains(grammar.getEpsilon())) {
        ...
        gt.blockOpen();
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool)
          gt.functionAssignCall("artCurrentSPPFNode", "artFindSPPFEpsilon", "artCurrentInputPairIndex");
        popSel("unimplementedEBNFClusteredDescriptor");
        gt.blockClose();
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.assign("artCurrentSPPFNode", "artDummySPPFNode");
       ...
       */
        ...
        if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.functionAssignCall("artCurrentSPPFNode", "artFindSPPFBaseNode",
            childNode.erL.toEnumString("L"), iftNode.erL.toEnumString("L"), "artCurrentInputPairIndex");
     ...
     */
      ...
      for (ARTGrammarInstance childNode = iftNode.getChild(); childNode != null; childNode = childNode.getSibling()) {
        if (optionBlock.predictivePops && childNode.isPredictivePop) {
          if (!optionBlock.suppressPopGuard) {
            gt.ifNotInSet("ARTSet" + grammar.getMergedSets().get(childNode.getGuard()), gt.inputAccessToken());
            jumpSelect("ARTX_DESPATCH");
          }
     ...
     */
      ...
      if (!(optionBlock.predictivePops && iftNode.isPostPredictivePop)) { // suppress testRepeat block if predictive pops in play
        iftNode.isTestRepeatLabel = true;
  ...
  void printTemplateCounts() {
    if (optionBlock.verbosityLevel > 0) System.out.printf(
  ...
  void printCodeAndSPPFLabelRec(ARTGrammarInstance node, boolean codeOnly) {
    if (node == null) return;
    // System.out.print("printCodeAndSPPFLabel() at node " + node + "\n");

    // if (codeOnly) cust.enumerationElement(node.toEnumString("L"));

    // Hacked during port to put eveything out
    if (codeOnly) {
  ...
  void writeARTLabelEnumeration() {
    ...
    for (ARTGrammarElementTerminal t : grammar.getTerminals()) {
      if (!(t instanceof ARTGrammarElementTerminalBuiltin) && !(t instanceof ARTGrammarElementTerminalFromNonterminal)) {
    ...
    // Now output 'non lexing' terminals
    for (ARTGrammarElementTerminal t : grammar.getTerminals()) {
      if (t instanceof ARTGrammarElementTerminalFromNonterminal) {
        // System.out.printf("Outputing enumeration for terminal from nonterminal %s\n", t);
        if (artFirstTerminal == null) artFirstTerminal = t;
        gt.enumerationElement(t.toEnumerationString(null));
      }
    }
    ...
    for (ARTGrammarElementTerminal t : grammar.getWhitespaces()) {
      // System.out.printf("Outputing enumeration for whitespace terminal %s\n", t);
      gt.enumerationElement(t.toEnumerationString(null));
    }
  ...
  void writeLexicaliseBuiltinInstances() {
    gt.functionVoidOpenThrows("artLexicaliseBuiltinInstances");
    for (ARTGrammarElementTerminal t : grammar.getTerminals())
      if (t instanceof ARTGrammarElementTerminalBuiltin) gt.lexerBuiltInInstance(t.getId(), optionBlock.parserName + "." + t.toEnumerationString(null));
    gt.functionClose("artLexicaliseBuiltinInstances");
    gt.newLine();
  }

  void writeLexicalisePreparseWhitespaceInstances() {
    gt.functionVoidOpenThrows("artLexicalisePreparseWhitespaceInstances");
    for (ARTGrammarElementTerminal b : grammar.getWhitespaces())
      if (b instanceof ARTGrammarElementTerminalBuiltin) gt.lexerWhitespaceBuiltinInstance(b.getId());

    gt.functionClose("artLexicalisePreparseWhitespaceInstances");
    gt.newLine();
  }

  void writeCodeBodiesGLL() throws ARTException {
    for (ARTGrammarElementNonterminal n : grammar.getNonterminals())
      writeCodeGLLRec(n.lhsInstance, null);
  }

  void writeParseGLLGenerated() throws ARTException {
    if (optionBlock.despatchMode == ARTModeDespatch.fragment) {
    ...
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool) {
      gt.indent();
      gt.getText().printf("tweSet = new ARTLexerBase(artHigher, artLonger, artInputString, ARTL_EOS, artLabelInternalStrings, %s, %s, %s);\n",
          optionBlock.lexLongestWithin, optionBlock.lexLongestAcross, optionBlock.lexPriority);
    }
    ...
    gt.functionCall("artLexicaliseUsingLongestMatch", "artInputString");
    gt.functionAssignCall("artLexCompleteTime", "artReadClock");

    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.functionAssignCall("artDummySPPFNode", "artFindSPPFInitial", "ARTL_DUMMY", "0", "0");
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.assign("artCurrentSPPFNode", "artDummySPPFNode");
    findGSSInitialSelect();
    gt.assign("artCurrentGSSNode", rootGSSNodeSelect());
    if (optionBlock.mGLL) gt.functionCall("artLoadDescriptorInitialMGLL");

    if (optionBlock.despatchMode == ARTModeDespatch.state || optionBlock.despatchMode == ARTModeDespatch.fragment
        || optionBlock.despatchMode == ARTModeDespatch.static_) {
      if (optionBlock.mGLL)
    ...
    if (optionBlock.despatchMode == ARTModeDespatch.static_) {
      gt.indent();
      jumpSelect("ARTX_DESPATCH_SWITCH");
      gt.indentUp();
    }

    if (optionBlock.despatchMode == ARTModeDespatch.dynamic) {
      gt.indent();
      jumpSelect(grammar.getDefaultStartNonterminal().lhsInstance.toEnumString("L"));
      gt.indentUp();
    }

    if (optionBlock.despatchMode == ARTModeDespatch.state) {
      gt.whileTrue("artSelectState");
      gt.caseOpen(optionBlock.despatchMode == ARTModeDespatch.state ? "artCurrentRestartLabel" : "artlhsL[artCurrentRestartLabel]");
      gt.indentUp();
    }
    ...
    gt.newLine();
    gt.indentUp();
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool) gt.functionCall("artCheckAcceptance");
    ...
    if (optionBlock.despatchMode == ARTModeDespatch.static_) printStaticSwitch();
  ...
  void writeSetInitialisation(HashMap<Set<ARTGrammarElement>, Integer> mergedSets) {
    for (Set<ARTGrammarElement> set : mergedSets.keySet()) {
      ...
      for (ARTGrammarElement e : set)
        if (e instanceof ARTGrammarElementTerminal)// System.out.printf("Writing merged set element %s number %d to parser%n", e.toString(),
                                                   // e.getElementNumber());
  ...
  void printLookupTablesNewRec(ARTGrammarInstance node) {
    ...
    if (node.isSPPFLabel || node.isCodeLabel || node.isReferredLabel || node.isTestRepeatLabel) {
      ...
      if (node.isLHS)
        gt.assignStringArrayElement("artLabelStrings", node.toEnumString("L"), ARTText.toLiteralString(node.getPayload().toString()));
      else
    ...
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool || optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool)
      if (node.isSPPFLabel || node.isCodeLabel || node.isReferredLabel || node.isTestRepeatLabel) {
        ...
        if (node.aL != node)
          gt.assignEnumerationArrayElement("artaL", node.toEnumString("L"), node.aL != null ? node.aL.toEnumString("L") : "ARTX_LABEL_EXTENT");
  ...
  void writeTableInitialise() {
    ...
    gt.newLine();
    gt.allocateBooleanArray("artTerminalsFromNonterminals", "ARTX_LABEL_EXTENT");
    for (ARTGrammarElementTerminal t : grammar.getTerminals())
      if (t instanceof ARTGrammarElementTerminalFromNonterminal)
        gt.assignBooleanArrayElement("artTerminalsFromNonterminals", "ARTTN_" + t.toEnumerationString(), "true");
    gt.newLine();
    gt.allocateBooleanArray("artNonterminalsDeclaredAsTerminals", "ARTX_LABEL_EXTENT");
    for (ARTGrammarElementNonterminal n : grammar.getNonterminalsDeclaredAsTerminals())
      gt.assignBooleanArrayElement("artNonterminalsDeclaredAsTerminals", n.toEnumerationString(null), "true");

    gt.newLine();
    gt.allocateBitSetArray("artLonger", "ARTX_LABEL_EXTENT + 1");
    for (int l = 0; l < grammar.getLonger().length; l++)
      for (int e = 0; e < grammar.getLonger().length; e++)
        if (grammar.getLonger()[l] != null && grammar.getLonger()[l].get(e)) gt.assignBitSetArrayElement("artLonger",
            grammar.getElementNumberMap().get(l).toEnumerationString(null), grammar.getElementNumberMap().get(e).toEnumerationString(null));
    gt.allocateBitSetArray("artHigher", "ARTX_LABEL_EXTENT + 1");
    for (int h = 0; h < grammar.getHigher().length; h++)
      for (int e = 0; e < grammar.getHigher().length; e++)
        if (grammar.getHigher()[h] != null && grammar.getHigher()[h].get(e)) gt.assignBitSetArrayElement("artHigher",
            grammar.getElementNumberMap().get(h).toEnumerationString(null), grammar.getElementNumberMap().get(e).toEnumerationString(null));
    gt.newLine();
    if (optionBlock.algorithmMode == ARTModeAlgorithm.gllGeneratorPool || optionBlock.algorithmMode == ARTModeAlgorithm.gllTWEGeneratorPool) {
      gt.allocateStringArray("artAnnotations", "ARTX_LABEL_EXTENT");
      gt.functionCall("artInitialiseStringArray", "artAnnotations", "0", "ARTX_LABEL_EXTENT", gt.nullName());
      gt.newLine();
  ...
  void writeConstructor(String id) {
    gt.constructorOpen(id, "ARTLexerBase", "artLexerBase");
    gt.functionCall("this", "null", "artLexerBase");
    gt.constructorClose(id);
    gt.newLine();

    gt.constructorOpen(id, "ARTGrammar", "artGrammar", "ARTLexerBase", "artLexerBase");
    gt.functionCall("super", "artGrammar", "artLexerBase");
    ...
    gt.assign("artSetExtent", "ARTX_EPSILON + 1");
    ...
    gt.assignString("artBuildOptions", ARTText.toLiteralString(optionBlock.toString()));

    gt.assign("artFIFODescriptors", optionBlock.FIFODescriptors ? "true" : "false");
  ...
  boolean writeRDEvaluatorTreeNode(ARTGrammarInstance iftNode, boolean isNonterminal, boolean isLeftChild, boolean hasAttributes) throws ARTException {
    ...
    case 3: {
      ...
      case TEAR:
        throw new ARTException("Tree builder: tear not yet implemented");

      default:
        throw new ARTException("Tree builder: unknown fold value found whilst writing tree builder");
      }
    }
      break;
    default:
      throw new ARTException("Tree builder: unknown tree level");
    }
  ...
  void writeRDEvaluatorProcessSemantics(ARTGrammarInstanceSlot node) {
    ...
    for (ARTGrammarInstance child = node.getChild(); child != null; child = child.getSibling()) {
      if (child instanceof ARTGrammarInstanceAction && !optionBlock.suppressSemantics) gt.text.printf("%s\n", child.getChild().getPayload());
    }
  }

  void writeRDEvaluatorProcessLeaf(ARTGrammarInstance node, boolean isLeftChild, ARTGrammarInstanceLHS lhsNode) throws ARTException {
  ...
  void writeRDEvaluatorRec(ARTGrammarInstance iftNode, ARTGrammarElementNonterminal currentNonterminal) throws ARTException {
    ...
    if (iftNode instanceof ARTGrammarInstanceLHS)
      ...
      throw new ARTException("Evaluator builder: unexpected SLOT node");

    else
      throw new ARTException("Evaluator builder: unknown tree node type " + iftNode);
  }
  ...
  void writeRDEvaluatorBodies(ARTGrammarElementNonterminal sym) throws ARTException {
    gt.text.printf(
    ...
    if (gt.targetLanguageName().equals("Java")) gt.text.printf("        default:\n"
        + "          throw new ARTException(\"%s: unhandled pack node \" + artPackedNode + \" with label \" + artSPPFPackedNodeLabel(artPackedNode) + \" - \" + artLabelInternalStrings[artSPPFPackedNodeLabel(artPackedNode)]);\n"
        + "        }\n" + "      }\n" + "    }\n", "ARTRD_" + ARTText.toIdentifier(sym.getId()));
    gt.indentDown();
    gt.indentDown();
  ...
  void writeRDEvaluator() throws ARTException {
    ...
    for (ARTGrammarElementNonterminal sym : grammar.getNonterminals()) {
      ...
      gt.text.printf(") throws ARTException {\n");
      gt.indent();
    ...
    if (grammar.isEBNF()) {
      gt.indent();
      gt.text.printf("public ARTGLLRDT artEvaluator() throws ARTException {\n");
      gt.indentUp();
      gt.indent();
      gt.text.printf("artText.println(\"artEvaluator not supported for EBNF grammars\");\n");

      gt.indent();
      gt.text.printf("return null;\n");
      gt.functionClose("artEvaluator");
      return;
    }

    if (grammar.getDefaultStartNonterminal().isContainsDelayedInstances() || grammar.getDefaultStartNonterminal().isHasDelayedInstances()
        || !grammar.getDefaultStartNonterminal().getAttributes().isEmpty()) {
      gt.indent();
      gt.text.printf("public ARTGLLRDT artEvaluator() throws ARTException {\n");
      gt.indentUp();
      gt.indent();
      gt.text.printf("ARTAT_%s_%s %s1 = new ARTAT_%s_%s();\n", grammar.getDefaultStartNonterminal().getModule().getId(),
          grammar.getDefaultStartNonterminal().getId(), grammar.getDefaultStartNonterminal().getId(), grammar.getDefaultStartNonterminal().getModule().getId(),
          grammar.getDefaultStartNonterminal().getId());
      gt.indent();
      gt.text.printf("return");
      gt.functionCall("artEvaluator", grammar.getDefaultStartNonterminal().getId() + "1");
      gt.functionClose("artEvaluator");

      gt.indent();
      gt.text.printf("\npublic ARTGLLRDT artEvaluator(ARTAT_%s_%s %s1) throws ARTException {\n", grammar.getDefaultStartNonterminal().getModule().getId(),
          grammar.getDefaultStartNonterminal().getId(), grammar.getDefaultStartNonterminal().getId());
      gt.indentUp();
      gt.indent();
      gt.text.printf("artRDT = new ARTGLLRDT(\"RDT\", artKindOfs, artLabelStrings, artAnnotations, artLexer.artInputString);\n");
      gt.indent();
      gt.text.printf(
          "ARTGLLRDTVertex artNewParent = new ARTGLLRDTVertex(artNextFreeNodeNumber++, new ARTGLLRDTPayload(artRDT, artSPPFNodeLeftExtent(artRootSPPFNode),artSPPFNodeRightExtent(artRootSPPFNode),artSPPFNodeLabel(artRootSPPFNode), %s1));\n",
          grammar.getDefaultStartNonterminal().getId());
      gt.indent();
      gt.text.printf("artRDT.setRoot(artNewParent);\n");
      gt.indent();
      gt.text.printf("boolean artNewWriteable = true;\n");
      gt.indent();
      gt.text.printf("artEvaluate(new ARTGLLRDTHandle(artRootSPPFNode), %s%d, artNewParent, artNewWriteable);\n", grammar.getDefaultStartNonterminal().getId(),
          1);
      gt.indent();
      gt.text.printf("return artRDT;\n");
      gt.functionClose("artEvaluator");
    }
      ...
      gt.text.printf("boolean artNewWriteable = true;\n");
      gt.indent();
      gt.text.printf("artEvaluate(new ARTGLLRDTHandle(artRootSPPFNode), null, artNewParent, artNewWriteable);\n");
      gt.functionAssignCall("artAttributeEvaluateCompleteTime", "artReadClock");
      gt.indent();
      gt.text.printf("return artRDT;\n");
      gt.functionClose("artEvaluator");
    }
