...
public class ARTValueTerm extends ARTValue {
  ...
  public ARTValueTerm(String string, ARTValueTerm... children) throws ARTException {
  ...
  private static ARTValueTerm term() throws ARTException {
    boolean isVariable = matchBackquoteCharacter(); // figure out use of interface
    if (isMetaCharacter()) throw new ARTException("in ARTValue.maketerms() unexpected metacharacter " + c + " at start of name");

    ARTValueTerm ret = new ARTValueTerm(new ARTValueString(name()));
    if (matchOpenParenthesisCharacter()) {
      ret.child = terms();
      if (!matchCloseParenthesisCharacter()) throw new ARTException("in ARTValueTerm.makeTerms(): expected ) found " + c);
    }

    return ret;
  }

  private static ARTValueTerm terms() throws ARTException {
  ...
  static public ARTValueTerm makeTerms(String stringTerm) throws ARTException {
    string = stringTerm;
    index = 0;
    c = string.charAt(index);
    skipWhitespace(); // Skip leading whitespace
    ARTValueTerm ret = terms();
    if (c != eos) throw new ARTException("in ARTValue.maketerms() unexpected characters following completet term");
    return ret;
  }
  ...
  public void addChild(ARTValueTerm v) throws ARTException {
    if (v.sibling != null) throw new ARTException("Attempt to add term as child which has non-null sibling field");
    if (child == null) {
      child = v;
      return;
    }
