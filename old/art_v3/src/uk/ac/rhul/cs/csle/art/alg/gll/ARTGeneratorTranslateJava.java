...
public class ARTGeneratorTranslateJava extends ARTGeneratorTranslate {
  ...
  void fileOpen(String filename, String nameSpace) {
    if (nameSpace != null) text.printf("package %s;\n\n", nameSpace);

    text.printf("import uk.ac.rhul.cs.csle.art.alg.gll.support.*;\n" + "import uk.ac.rhul.cs.csle.art.lex.*;\n" + "import uk.ac.rhul.cs.csle.art.manager.*;\n"
  ...
  void classOpen(String id, String superClassName) {
    className = id;
    text.printf("public class %s extends %s {\n", id, superClassName);
  }
  ...
  @Override
  void functionVoidOpenThrows(String id, String p1Type, String p1, String p2Type, String p2) {
    indent();
    text.printf("public void %s(%s %s, %s %s) throws ARTException {\n", id, p1Type, p1, p2Type, p2);
    indentUp();
  }

  @Override
  void functionVoidOpenThrows(String id) {
    indent();
    text.printf("public void %s() throws ARTException {\n", id);
    indentUp();
  }
  ...
  void allocateBitSetArray(String id, String extent) {
    ...
    indent();
    text.printf("for (int i = 0; i < " + extent + "; i++) " + id + "[i] = new ARTBitSet();\n");
  }

  @Override
  void assignBitSetArrayElement(String id, String index, String value) {
    indent();
