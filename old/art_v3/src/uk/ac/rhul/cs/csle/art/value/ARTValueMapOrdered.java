...
public class ARTValueMapOrdered extends ARTValueCollection {
  ...
  public ARTValue insertKey(ARTValue key, ARTValue value) throws ARTValueException {
    if (payload.containsKey(key)) throw new ARTValueException("ARTValueMapOrdered insertion of already existing key " + key);
  ...
  public ARTValue deleteKey(ARTValue key) throws ARTValueException {
    if (!payload.containsKey(key)) throw new ARTValueException("ARTValueMapOrdered deletion of key that is not in the map" + key);
    payload.remove(key);
    return this;
  }
  ...
  public ARTValue valueKey(ARTValue key) throws ARTValueException {
    if (payload.containsKey(key))
      ...
      throw new ARTValueException("ARTValueMapOrdered value requested for nonexistent key " + key);
  }
