...
import uk.ac.rhul.cs.csle.art.manager.grammar.ARTFold;
...
public class ARTGrammarInstance extends ARTTreeVertexDoublyLinked implements Comparable<ARTGrammarInstance> {
  ...
  public String toGrammarSlotStringRec(ARTGrammarInstance slotNode, String slotDesignator, String s, boolean stopAfterSlot) {

    // Preorder actions
    if (this instanceof ARTGrammarInstanceDiff) s += "/" + optionalSpace();

    if (this instanceof ARTGrammarInstanceTerminal) s += getPayload().toString() + "" + optionalSpace();

    if (this instanceof ARTGrammarInstanceNonterminal) s += getPayload().toString() + "" + optionalSpace();
    ...
    for (ARTGrammarInstance child = getChild(); child != null; child = child.getSibling()) {
      s = child.toGrammarSlotStringRec(slotNode, slotDesignator, s, stopAfterSlot);
  ...
  public String toGrammarString(String slotDesignator) {
    ...
    if (production != null)
      return production.toGrammarSlotStringRec(this, slotDesignator, lhsString + " ::= ", false);
    else
      return "(" + key + ", " + payload + ")";
  }
