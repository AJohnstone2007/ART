...
public class ARTGLLRDT extends ARTTree {
  ...
  public ARTGLLRDT(String label, int[] artKindOfs, String[] artLabelStrings, String[] artAnnotations, String artCharacterStringInput) {
    super(label);
    this.kindOfs = artKindOfs.clone();
    this.labelStrings = artLabelStrings.clone();
    this.annotations = artAnnotations.clone();
  ...
  @Override
  public void printDot(String filename) throws ARTException {
    this.printDot(new File(filename));
  }

  @Override
  public void printDot(File file) throws ARTException {
    String filename = "??Unnown filename??";
    try {
      filename = file.getCanonicalPath();
      this.printDot(new PrintWriter(file));
    } catch (IOException e) {
      throw new ARTException("Unable to open output file " + filename + "\n");
    }
  }
