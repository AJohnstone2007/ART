...
public class ARTSTLASCIIParser extends ARTSTLParser {
  ...
  public ARTSTLASCIIParser(String filename) throws ARTException {
    System.out.println("Opening ASCII STL " + filename);
    try {
      scanner = new Scanner(new File(filename));
    } catch (FileNotFoundException e) {
      throw new ARTException("Unable to open file " + filename);
    }
    ...
    if (scanner.next("endsolid") == null) throw new ARTException("Ill-formed ASCII STL input");
    ...
    try {
      ...
      throw new ARTException("Unable to open ASCII STL file " + filename);
    }
    scanner.next("solid");
