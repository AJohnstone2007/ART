...
static void rdp_print_parser_subproduction(rdp_data * prod, rdp_data * primary, int promote_epsilon, char * default_action)
{
  ...
  if (prod->lo == 0 &&(rdp_dir_tree || default_action != NULL))
  {
    ...
    if (rdp_dir_tree)
    {
      ...
      else if (promote_epsilon == PROMOTE_ABOVE)
      {
        /* add an epsilon tree node */
        rdp_indent();
        if (rdp_dir_annotated_epsilon_tree)
          text_printf("if (rdp_tree_update) {rdp_tree_node_data *temp = rdp_add_parent(NULL, rdp_tree); temp->id = \"#: %s\"; temp->token = SCAN_P_ID;}\n", prod->id + 4);
        else
          text_printf("if (rdp_tree_update) {rdp_tree_node_data *temp = rdp_add_parent(NULL, rdp_tree); temp->id = NULL; temp->token = SCAN_P_ID;}\n");
      }
    }
