...
{
  char *p_char, *p_char_esc, *p_string, *p_string_esc, *p_comment, *p_comment_visible,
	*p_comment_nest, *p_comment_nest_visible, *p_comment_line, *p_comment_line_visible,
	*p_ignore;
  ...
  text_insert_string("SCAN_P_KEYWORD");
  ...
  while (p != NULL)
  {
    if (p->kind == K_TOKEN || p->kind == K_EXTENDED)
    {
      ...
      if (rdp_is_valid_C_id(p->id))
	text_insert_string(p->id);
      else
      {
	char * c = p->id;

	text_insert_integer(p->token_value);
	...
	{
	  case SCAN_P_CHAR: p->extended_enum = p_char; break;
	  case SCAN_P_CHAR_ESC: p->extended_enum = p_char_esc; break;
	  case SCAN_P_STRING: p->extended_enum = p_string; break;
	  case SCAN_P_STRING_ESC: p->extended_enum = p_string_esc; break;
	  case SCAN_P_COMMENT: p->extended_enum = p_comment; break;
	  case SCAN_P_COMMENT_VISIBLE: p->extended_enum = p_comment_visible; break;
	  case SCAN_P_COMMENT_NEST: p->extended_enum = p_comment_nest; break;
	  case SCAN_P_COMMENT_NEST_VISIBLE: p->extended_enum = p_comment_nest_visible; break;
	  case SCAN_P_COMMENT_LINE: p->extended_enum = p_comment_line; break;
	  case SCAN_P_COMMENT_LINE_VISIBLE: p->extended_enum = p_comment_line_visible; break;
	}
      }
      else p->extended_enum = p_ignore;
    }
    p = (rdp_data*) symbol_next_symbol_in_scope(p);
  }
}
...
static void rdp_print_parser_param_list(rdp_param_list *params, int definition)
{
  text_printf("(");
 ...
 case K_REAL:
  ...
  case K_LIST:
	...
	text_printf(":'%s'", prod->supplementary_token->id);
      ...
      text_printf("%s ", prod->id);
    break;
  default: text_message(TEXT_FATAL, "Unexpected kind found\n");
  }
}
...
void rdp_print_header(char *headerfilename)
{
  ...
  if (*headerfilename == '-')
    ...
    text_message(TEXT_FATAL, "Can't open header output file '%s' for writing\n", headerfilename);
  ...
  text_printf(
	  ...
	  text_uppercase_string(rdp_sourcefilename),
	  text_uppercase_string(rdp_sourcefilename)
	  );
  ...
  rdp_print_parser_param_list(rdp_start_prod->params, 1);
...
static void rdp_print_parser_subproduction(rdp_data * prod, rdp_data * primary)
{
  ...
  if (prod->supplementary_token != NULL)
  {
   ...
   rdp_indent();
   if (rdp_dir_build_tree)
     text_printf("SCAN_ADD_CHILD ");
   text_printf("scan_();\n");  /* skip list token */
  }
  else
  if (prod->hi != 1)
...
static void rdp_print_parser_item(rdp_data * prod, rdp_data * primary, char *return_name, rdp_param_list* actuals, int do_not_instantiate)
{
  ...
  case K_TOKEN:
    ...
    text_printf(", &%s_stop);\n" , primary->id);
    rdp_indent();
    if (!do_not_instantiate && rdp_dir_build_tree)
      text_printf("SCAN_ADD_CHILD ");
    text_printf("scan_();\n");
    break;
  case K_CODE:
  ...
  case K_PRIMARY:
    ...
    {
      text_printf("scan_retain_parent = 1;\n");
      rdp_indent();
    }
    ...
    if (!(prod->code_only && actuals == NULL))
      rdp_print_parser_param_list(actuals, 0);
    text_printf(";\n");
    if (do_not_instantiate)
    {
      rdp_indent();
      text_printf("scan_retain_parent = 0;\n");
    }
    break;
  case K_SEQUENCE:
    ...
    break;
  case K_LIST:
    rdp_print_parser_subproduction(prod, primary);
    break;
  default: text_message(TEXT_FATAL, "Unexpected kind found\n");
  }
}
...
static void rdp_print_parser_sequence(rdp_data * production, rdp_data * primary)
{
  ...
  while (list != NULL)
  {
    rdp_print_parser_item(list->production, primary, list->return_name, list->actuals, list->do_not_instantiate);
    list = list->next;
  }
}
...
static void rdp_print_parser_alternate(rdp_data * production, rdp_data * primary)
{
  ...
  else
  {
	...
	text_message(TEXT_FATAL, "Internal error - expecting alternate production\n");
...
static void rdp_print_parser_primaries(void * base)
{
  ...
  while (temp != NULL)
  {
	...
	text_printf("*");
      ...
      rdp_print_parser_param_list(temp->params, 1);
	...
	rdp_print_locals(temp);
      ...
      /* If we are tree building, add a placemaker for the tree and add child node */
      if (rdp_dir_build_tree && !temp->never_instantiate)
	text_printf("  void *parent = scan_parent;   /* remember tree parent */\n\n"
		    "  if (!scan_retain_parent && rdp_pass == RDP_PASSES)\n"
		    "  {\n"
		    "    scan_parent = graph_insert_node_child(sizeof(scan_data), 0, parent);\n"
		    "    ((scan_data*) scan_parent)->id = \"%s\";\n"
		    "  }\n\n", temp->id
		   );
	...
	text_printf("  text_message(TEXT_INFO, \"Entered '%s'\\n\");\n\n", temp->id);
      ...
      text_printf("  if (");
      rdp_print_parser_test(temp->id, &temp->first, temp->contains_null ? NULL : temp->id);
      text_printf(")\n  {\n");
      rdp_indentation = 2;
      ...
      /* If tree building, restore parent */
      if (rdp_dir_build_tree && !temp->never_instantiate)
...
void rdp_print_parser(char *outputfilename, void * base)
{
  ...
  if (*outputfilename == '-')
    ...
    text_message(TEXT_FATAL, "Can't open parser file '%s' for writing\n", outputfilename);
  ...
  while (temp != NULL)
  {
	...
	if (temp->params != NULL)
	  rdp_print_parser_param_list(temp->params, 0);
	text_printf(" ");
	rdp_print_parser_alternate(temp, temp);
      }
      else
  ...
  text_printf("  if (rdp_sourcefilename == NULL)\n"
  ...
  text_printf("  for (rdp_pass = 1; rdp_pass <= RDP_PASSES; rdp_pass++)\n  {\n");

  text_printf(
	  "    text_echo(rdp_line_echo_all || (rdp_line_echo && rdp_pass == RDP_PASSES));\n"
	  "    if (text_open(text_default_filetype(rdp_sourcefilename, \"%s\")) == NULL)\n"
  ...
  rdp_print_parser_param_list(rdp_start_prod->params, 0);
...
void rdp_dump_extended(void * base)
{
  ...
  while (temp != NULL)
  {
	...
	while (list != NULL)
	{
	  ...
	  rdp_print_parser_param_list(list->actuals, 0);
