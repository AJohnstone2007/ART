...
unsigned
 rdp_dir_build_tree = 0,       /* BUILD_TREE flag */
...
rdp_data *rdp_find(char *id, kind_type kind, symbol_type symbol)
{
  ...
  if (kind == K_CODE)
    table = codes;
  if (kind == K_TOKEN || kind == K_EXTENDED)
    table = tokens;
  else table = rdp;
...
void rdp_pre_parse(void)
{
  ...
  rdp_find("EOLN", K_INTEGER, RDP_ANY)->token_value = SCAN_P_EOLN;
  ...
  while (temp != NULL)          /* scan over all productions */
  {
    if (temp->kind == K_TOKEN || temp->kind == K_EXTENDED)
    {
      ...
      *hi = temp->id;
      ...
      if (!rdp_is_valid_C_id(hi))  /* ignore identifiers */
      {
...
void rdp_post_parse(char *outputfilename, char force)
{
  void
