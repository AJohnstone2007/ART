...
void scan_(void)
{                               /* get the next lexeme */
  char *start = text_top;       /* remember start of symbol */
  scan_data *s;
  int nestable;
  unsigned nestlevel;
  char *close;
  ...
  do
  {
    ...
    while (text_char != EOF &&
	   !(scan_newline_visible && text_char == '\n') &&
	   isspace(text_char)
      )
	...
	if (text_char == 'x' || text_char == 'X')
	{
      ...
      while (hex ? isxdigit(text_char) : isdigit(text_char))
      {
        text_insert_char(text_char);
        text_get_char();
      }
    ...
    {                           /* process non-alphanumeric symbol */
      ...
      else
      {
        ...
        *this_sym = NULL;
        ...
        while (last_sym = this_sym, text_insert_char(text_char), *text_top = '\0',
    ...
    {
    case SCAN_P_CHAR:
      text_insert_char(text_char);  /* insert character in string table */
      text_insert_char(0);      /* terminate string */
      text_get_char();
      SCAN_CAST->id = start;
	...
	/* translate all C escapes. Anything else returns escaped character */
	  ...
	  text_top = start;     /* scrub from buffer */
        ...
        default:                /* any other quoted character returns itself */
          text_insert_char(text_char);  /* insert ordinary character */
          text_get_char();
          break;
        }
      }
      ...
      {
        text_insert_char(text_char);  /* just insert character in string table */
        text_insert_char(0);
        text_get_char();
      }
      text_insert_char(0);
    ...
    case SCAN_P_STRING:
      do
      {
        while (text_char != *(SCAN_CAST->id))
        {
          ...
          text_insert_char(text_char);  /* add character to string */
          text_get_char();      /* get next character */
        }
        text_get_char();        /* get character after close */
      }
      while (text_char == *(SCAN_CAST->id) ?
             text_insert_char(text_char), text_get_char(), 1
