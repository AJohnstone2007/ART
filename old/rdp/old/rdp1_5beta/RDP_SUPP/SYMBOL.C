...
typedef struct link_sym_node
{
  void *symbol;
  struct link_sym_node *next;
}link_sym;


typedef struct link_tab_node
{
  struct link_sym_node *symbols;
  struct link_tab_node *next;
  void *table;
}link_tab;
...
{
  link_tab *links;/* pointer to list of list of links */
...
{
  void *header = symbol_get_link_header(symbol, table);

  return header == NULL ? NULL : ((link_tab *) header)->symbols;
}

void *symbol_get_linked_symbol(const void *link) /* return symbol from link */
{
  return ((link_sym*) link)->symbol;
}

/* try and find a link from a symbol to a table. Return NULL if not found */
void *symbol_get_link_header(const void *symbol, const void *table)
{
  ...
  /* search up table chain until we find a pointer to table */
  temp_table = s->links;
  while (temp_table != NULL && temp_table->table != table)
...
void symbol_link(void *symbol, void *table, void *dst)
{
  symbol_ *s = SYMBOL - 1;

  link_tab *temp_table = (link_tab*) symbol_get_link_header(symbol, table);
  link_sym *temp_symbol;

  if (temp_table == NULL)       /* table was not found so insert at head of list */
  {
    /* make a new table node */
    temp_table = (link_tab *) mem_malloc(sizeof(link_tab));
    temp_table->table = table;
    temp_table->symbols = NULL;
    temp_table->next = s->links;
    s->links = temp_table;
  }

  /* now we have located the table, so search for pre-existing link */
  temp_symbol = temp_table->symbols;
  while (temp_symbol != NULL && temp_symbol->symbol != dst)
    temp_symbol = (link_sym *) symbol_next_link(temp_symbol);

  if (temp_symbol == NULL)
  {
    /* make a new link node */
    temp_symbol = (link_sym *) mem_malloc(sizeof(link_sym));
    temp_symbol->symbol = dst;    /* point to destination symbol */
    temp_symbol->next = temp_table->symbols;  /* insert at head of table list */
    temp_table->symbols = temp_symbol;
  }
}
...
void *symbol_next_link(void *link)
{
  return ((link_sym *) link)->next;
}
...
{
  link_tab *table = ((symbol_ *) ((symbol_ *) symbol - 1))->links;

  while (table != NULL)
  {
    link_sym *symbol = table->symbols;

    while (symbol != NULL)
    {
      text_printf("-> ");
      (*((symbol_table *) table->table)->print) (symbol->symbol);
      text_printf("\n");
      symbol = symbol->next;
    }
    table = table->next;
...
void symbol_sort_links(void *links, int (*fcmp)(const void *, const void *))
{
  ...
  /* make the sort array */
  sort_array = (void **) mem_malloc(count * sizeof(void*));

  /* load the sort array with the links */
  count = 0;
  temp = links;
  while(temp != NULL)
  {
    sort_array[count++] = ((link_sym*) temp)->symbol;
    temp = symbol_next_link(temp);
  }

  /* quick sort the array */
  qsort(sort_array, count, sizeof(void*), fcmp);

  /* unload the links from the sort array */
  count = 0;
  temp = links;
  while(temp != NULL)
  {
    ((link_sym*) temp)->symbol = sort_array[count++];
    temp = symbol_next_link(temp);
  }
