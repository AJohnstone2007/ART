...
void scan__echo_line(FILE * fname)
{
  ...
  {
    temp = first_char;
    fprintf(fname, "        ");/* Don't print line numbers in text mode */
  ...
  else
  {
    if (*(first_char - 1) == EOF_CHAR)  /* Don't dump if at end of file */
      return;
    fprintf(fname, "%6i: ", current_source->linenumber); /* Print present line number */
...
static void scan__echo_line_pointer(FILE * f)
{
  char *temp;
  ...
  if (f != NULL)                /* skip echoing for null file */
  {
    if (!scan__flags.line_echoed) /* echo line if it hasn't already been output */
      scan__echo_line(f);
    fprintf(f, "******* ");/* print error highlight */
    if (current_source->is_text)
    {
...
int scan__error_echo(const char *fmt,...) /* Print a formated error message */
{
  ...
  scan__echo_line_pointer(MESSAGES);

  fprintf(MESSAGES, "Error %s: ", current_source->name);
...
int scan__warning_echo(const char *fmt,...) /* Print a formated error message */
{
  ...
  fprintf(MESSAGES, "Warning %s: ", current_source->name);
...
/****************************************************************************
*
* Insert the ASCII representation of a INTEGER into the text buffer
*
****************************************************************************/
void scan__insert_integer(unsigned n)
{
  if (n > 9)
    scan__insert_integer(n / 10); /* recursively handle multi-digit numbers */
  scan__insert_char(n % 10 + '0');
}

/****************************************************************************
*
* Insert a string followed by an underscore followed by a INTEGER
*
****************************************************************************/
...
{
  fprintf(MESSAGES, "Scanned ");
  switch (s->token)
  {
  case P_BAD:
    fprintf(MESSAGES, "BAD     ");
    break;
  case P_IGNORE:
    fprintf(MESSAGES, "IGNORE  ");
    break;
  case P_ID:
    fprintf(MESSAGES, "ID      ");
    break;
  case P_NEW_ID:
    fprintf(MESSAGES, "NEW_ID  ");
    break;
  case P_ALT_ID:
    fprintf(MESSAGES, "ALT_ID  ");
    break;
  case P_INTEGER:
    fprintf(MESSAGES, "INTEGER  ");
    break;
  case P_REAL:
    fprintf(MESSAGES, "REAL     ");
    break;
  case P_EOF:
    fprintf(MESSAGES, "EOF     ");
    break;
  case P_EOLN:
    fprintf(MESSAGES, "EOLN    ");
    break;
  default:
    if (s->token < P_TOP)
      fprintf(MESSAGES, "??????? ");
    else
      fprintf(MESSAGES, "KEYWORD ");
  }
  fprintf(MESSAGES, "token=%3u  id '%s'\n", s->token, s->id);
}
  ...
  if (current_source == NULL)   /* Has the last file been shut? */
    ...
    do
    {
      ...
      {                         /* read a number of some sort */
        ...
        scan__sym->id = scan__text; /* remember start position */
        scan__sym->token = P_INTEGER; /* assume integer */
        ...
        while (hex ? isxdigit(*next_char) : isdigit(*next_char))
        {
          scan__insert_char(*next_char);
          scan__get_char();
        }
        ...
        {
	  /* get decimal */
          if (*next_char == '.' && *(next_char-1) != '.' /* Pascal sneakiness */)
          {
            scan__sym->token = P_REAL;
	    do
            {
              scan__insert_char(*next_char);
              scan__get_char();
        ...
        scan__insert_char('\0');/* terminate string */
        if (scan__sym->token == P_INTEGER)
          scan__sym->data.u = strtoul(scan__sym->id, NULL, 0);
        else
          scan__sym->data.r = strtod(scan__sym->id, NULL);
      }
      else
	...
	if (*next_char == EOF_CHAR)
        ...
        else
        {
          ...
          *this_sym = NULL;
          ...
          if (scan__sym->data.p != NULL)  /* must be extended keyword */
          {
            struct extended_keyword_data *e = (struct extended_keyword_data *) (scan__sym->data.p);
	    int nestable = 0,
             nestlevel = 1;
		  ...
		  break;
                ...
                if (*next_char == *(e->close))  /* found escape character */
                {
                  long int temp;
                  char *start;
		    ...
		    break;
                  ...
                  case 'X':     /* hexadecimal */
                    ...
                    scan__text = 0; /* change last character to a null */
                    temp = strtol(start, NULL, 16);
                    scan__text = start;  /* scrub from buffer */
                    if (temp>255)
                      scan__warning_echo("Hex escape sequence overflows eight bits: wrapping");
                    scan__insert_char((char) (temp % 255));
                    break;
                  case '0':
                  case '1':
                  case '2':
		    ...
		    while (*next_char >= '0' && *next_char<= '7');
                    ...
                    scan__text = start;  /* scrub from buffer */
                    if (temp>255)
                      scan__warning_echo("Octal escape sequence overflows eight bits: wrapping");
                    scan__insert_char((char) (temp % 255));
                    break;
                  default: /* any other quoted character returns itself */
                    scan__insert_char(*next_char);  /* insert ordinary character */
                    scan__get_char();
                    break;
                  }
                }
...
{
  s->data.p = crash__malloc(sizeof(struct extended_keyword_data), "extended keyword data");

  ((struct extended_keyword_data *) (s->data.p))->kind = kind;

  ((struct extended_keyword_data *) (s->data.p))->close = (char *) close;
}
...
char *scan__default_filetype(char *fname, const char *ftype)
{
  ...
  fullname = (char *) crash__malloc(strlen(fname) + strlen(ftype) + 2, "file name");
...
/* strip the filetype from fname and add ftype. Ignore leading . and ..  */
char *scan__add_filetype(char *fname, const char *ftype)
{
  ...
  fullname = (char *) crash__malloc(length + strlen(ftype) + 2, "file name");
...
void scan__init(unsigned max_text, unsigned max_errors, unsigned max_warnings)
{
  ...
  text = (char *) crash__malloc(maxtext, "scanner text buffer");  /* allocate text buffer */
...
FILE *scan__open_file(char *s)
{
  ...
  if (handle != NULL)
  {
    struct source_list *temp = (struct source_list *) crash__calloc(sizeof(struct source_list), "scanner file node");
...
char *scan__load_file(char *s)
{
  ...
  if (*s == '-')
    ...
    temp = fopen(scan__default_filetype(s, "bnf"), "r");
...
void scan__close(void) /* never close the last file! */
{
  ...
  if (current_source->last == NULL) /* don't close last file ! */
    scan__sym->token = P_EOF;
  else
  {
    ...
    {
      scan__info("        %s", current_source->name);
      scan__echo_line(MESSAGES);  /* reecho line in case there are errors */
    }
  }
}
